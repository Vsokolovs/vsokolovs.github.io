...
const DEFAULT_DATA = {
  ...
  title: "Practice Lead: Data & Analytics",
  ...
  contact: {
    phone: "+37126307373",
    email: "vladislavs.sokolov@gmail.com",
    location: "Jurmala, Latvia",
    linkedin: "",
    github: ""
  },
  skills: [
    "Management Skills", "Product Ownership", "Agile Delivery", "SQL", "Python", "Azure Stack", "Databricks", "dbt",
    "Leadership", "Business & Communication", "Solution Design", "Apache Kafka", "Apache Spark", "Polars", "Pandas",
    "PostgreSQL", "MySQL", "GDScript", "Godot", "Apache Iceberg", "Microsoft Fabric", "PowerBI", "Looker"
  ],
  experience: [
    {
      date: "2022 - Present",
      company: "Emergn",
      role: "Lead Data Engineer -> D&A Practice Lead",
      description: "Took data owner responsibilities for several client data initiatives: defining scope, aligning with business stakeholders, managing data governance and implementation, preparing roadmaps, and prioritizing delivery for engineering teams. Owned end-to-end delivery of data products: data ingestion, data governance, transformation logic, quality processes, and BI output."
    },
    {
      date: "2019 - 2022",
      company: "Luminor Bank",
      role: "Mid-Senior Data Engineer / Data Analyst",
    ...
    {
      date: "2017 - 2019",
      company: "L'Oreal Baltics",
      role: "Corporate Finance Controller",
      description: "Delivered analytical insights, supported system implementations, handled cost control and budgeting. Led rollout of new accounting and travel expense systems."
    }
  ],
  projects: [
    {
      title: "Enterprise Data Platform",
      tech: "Azure · Databricks · dbt",
      description: "Designed and delivered a scalable data platform serving multiple business units with governed, high-quality data products.",
      link: ""
    }
  ]
};
...
async function loadData() {
  try {
    ...
    const local = localStorage.getItem("cv_working");
...
function renderExperience() {
  ...
  timeline.innerHTML = DATA.experience.map((item) =>
    `<div class="timeline-item"><div class="timeline-date">${esc(item.date)}</div><div class="timeline-company">${esc(item.company)}</div><div class="timeline-role">${esc(item.role)}</div><p class="timeline-desc">${esc(item.description)}</p></div>`
  ).join("");
}
...
function renderSkills() {
  ...
  if (!list) return;
  list.innerHTML = DATA.skills.map((skill) => `<span class="skill-tag">${esc(skill)}</span>`).join("");
}
...
function renderContacts() {
  ...
  let html = "";
  if (DATA.contact.phone) html += `<div class="contact-item">${ICONS.phone}<span>${esc(DATA.contact.phone)}</span></div>`;
  if (DATA.contact.email) html += `<div class="contact-item">${ICONS.email}<a href="mailto:${esc(DATA.contact.email)}">${esc(DATA.contact.email)}</a></div>`;
  if (DATA.contact.location) html += `<div class="contact-item">${ICONS.location}<span>${esc(DATA.contact.location)}</span></div>`;
  if (DATA.contact.linkedin) html += `<div class="contact-item">${ICONS.linkedin}<a href="${esc(DATA.contact.linkedin)}" target="_blank" rel="noopener">LinkedIn</a></div>`;
  if (DATA.contact.github) html += `<div class="contact-item">${ICONS.github}<a href="${esc(DATA.contact.github)}" target="_blank" rel="noopener">GitHub</a></div>`;

  list.innerHTML = html || '<div class="empty-state">No contact details yet.</div>';
}
...
function renderProjects() {
  ...
  if (!DATA.projects || !DATA.projects.length) {
    grid.innerHTML = '<div class="empty-state">Projects coming soon.</div>';
    return;
  }

  grid.innerHTML = DATA.projects.map((project) =>
    `<div class="project-card"><h3>${esc(project.title)}</h3><div class="project-tech">${esc(project.tech)}</div><p>${esc(project.description)}</p>${project.link ? `<a class="project-link" href="${esc(project.link)}" target="_blank" rel="noopener">View Project -></a>` : ""}</div>`
  ).join("");
}
...
function renderHomePage() {
  ...
  setText("profileText", DATA.profile);
  renderExperience();
  renderSkills();
  renderContacts();
}
